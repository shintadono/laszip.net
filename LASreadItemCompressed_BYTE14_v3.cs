//===============================================================================
//
//  FILE:  lasreaditemcompressed_byte_v3.cs
//
//  CONTENTS:
//
//    Implementation of LASreadItemCompressed for BYTE14 items (version 3).
//
//  PROGRAMMERS:
//
...
{
	class LASreadItemCompressed_BYTE14_v3 : LASreadItemCompressed
	{
		...
		public override bool chunk_sizes()
		{
			...
			for (uint i = 0; i < number; i++)
			{
				...
				num_bytes_Bytes[i] = (int)BitConverter.ToUInt32(buffer, 0);
			}
		...
		public override bool init(laszip_point item, ref uint context) // context is only read
		{
			...
			for (uint i = 0; i < number; i++)
			{
				if (requested_Bytes[i])
				{
					...
					{
						if (instream.Read(bytes, num_bytes, num_bytes_Bytes[i]) != num_bytes_Bytes[i]) throw new EndOfStreamException();
