//===============================================================================
//
//  FILE:  lasreaditemcompressed_wavepacket14_v4.cs
//
//  CONTENTS:
//
//    Implementation of LASreadItemCompressed for WAVEPACKET14 items (version 4).
//
//  PROGRAMMERS:
//
...
{
	class LASreadItemCompressed_WAVEPACKET14_v4 : LASreadItemCompressed
	{
		...
		public override bool chunk_sizes()
		{
			...
			num_bytes_wavepacket = (int)BitConverter.ToUInt32(buffer, 0);
		...
		public override bool init(laszip.point item, ref uint context) // context is only read
		{
			...
			if (requested_wavepacket)
			{
				...
				{
					if (instream.Read(bytes, 0, num_bytes_wavepacket) != num_bytes_wavepacket) throw new EndOfStreamException();
